(() => {
  ...
  const state = {
    ...
    view: 'overview',
    data: {},
    menuConfigs: {} // cached field/button metadata loaded from menu management
  };
  ...
  const setLanguage = lang => {
    ...
    if (state.token) loadNavigation().finally(() => renderView(state.view));
    renderIcons();
  };
  ...
  async function getMenuConfig(code) {
  ...
  function readPath(obj, path) { return String(path || '').split('.').reduce((v, key) => v == null ? undefined : v[key], obj); }
  function localizedMeta(meta, prefix = 'label') { return state.lang === 'ar-TN' ? (meta[prefix + 'Ar'] || meta[prefix + 'En'] || meta[prefix + 'Zh']) : state.lang === 'en' ? (meta[prefix + 'En'] || meta[prefix + 'Zh']) : meta[prefix + 'Zh']; }
  ...
  function navLabel(item) { return state.lang === 'ar-TN' ? (item.nameAr || item.nameEn || item.nameZh) : state.lang === 'en' ? (item.nameEn || item.nameZh) : item.nameZh; }
  function renderNavigation(items) {
    const active = items.filter(m => m.status !== 'INACTIVE');
    const byParent = active.reduce((map, item) => { (map[item.parentCode || '__root'] = map[item.parentCode || '__root'] || []).push(item); return map; }, {});
    ...
    if (html) { $('#main-nav').innerHTML = html; renderIcons(); applyPermissions(); }
  }
  async function loadNavigation() { try { const response = await api('/navigation'); renderNavigation(response.data || []); } catch (e) { /* static fallback remains available */ } }
  ...
  function renderView(view) {
    ...
    // Auto-expand parent nav group of active view, collapse all others
    ...
    if (activeLink) {
      ...
      // Expand the parent group of the active view
  ...
  async function loadProducts(page) {
    ...
    try {
      ...
      const configured = (config?.fields || []).filter(f => f.status === 'ACTIVE' && f.visibleList);
      const headers = configured.length ? configured.map(f => ({ key: f.fieldCode, label: localizedMeta(f), sortable: true })) .concat([{ key:'actions', label:t('actions'), sortable:false }]) : defaultHeaders;
      // Client-side sort
      ...
      const rows = configured.length ? p.items.map(v => `<tr>${configured.filter(f => f.visibleList).map(f => { const value = readPath(v, f.fieldPath); return `<td class="${f.fieldCode === 'code' ? 'code' : ''}">${f.fieldCode === 'name' ? `<span class="cell-title">${escVal(v.nameZh)}</span><span class="cell-sub">${escVal(v.nameEn || v.nameAr)}</span>` : escVal(value)}</td>`; }).join('')}<td class="table-actions">${btn('product-detail', icon('eye'), 'ghost', 'ENGINEERING_READ')} ${btn('product-sync', icon('refresh-cw'), 'ghost', 'INTEGRATION_WRITE')}</td></tr>`) : p.items.map(v => `<tr><td class="code">${escVal(v.code)}</td><td><span class="cell-title">${escVal(v.nameZh)}</span><span class="cell-sub">${escVal(v.nameEn || v.nameAr)}</span></td><td>${escVal(v.productType)}</td><td>${escVal(v.specification)}</td><td>${escVal(v.productModel)}</td><td>${escVal(v.customerPartNumber)}</td><td>${escVal(v.minPackagingQty)}</td><td>${escVal(v.unit)}</td><td>${statusPill(v.source || 'SAP')}</td><td>${formatDate(v.sapLastSyncAt)}</td><td class="table-actions">${btn('product-detail', icon('eye'), 'ghost', 'ENGINEERING_READ')} ${btn('product-sync', icon('refresh-cw'), 'ghost', 'INTEGRATION_WRITE')}</td></tr>`);
  ...
  async function loadBatches(page) {
    ...
    try {
      ...
      node.innerHTML = dataTable(
        headers,
        p.items.map(v => `<tr><td class="code">${escVal(v.batchNo)}</td><td><span class="cell-title">${escVal(v.productCode)}</span><span class="cell-sub">${escVal(v.productName || '')}</span></td><td>${escVal(v.plant)}</td><td>${statusPill(v.batchStatus)}</td><td>${escVal(v.quantity)} ${escVal(v.unit)}</td><td>${formatDate(v.manufactureDate)}</td><td>${formatDate(v.expirationDate)}</td><td>${formatDate(v.shelfLifeExpirationDate)}</td><td>${escVal(v.supplierBatch)}</td><td>${escVal(v.inspectionLot)}</td><td>${formatDate(v.sapLastSyncAt)}</td><td class="table-actions">${btn('batch-detail', icon('eye'), 'ghost', 'QUALITY_READ')}</td></tr>`),
        p.sortKey, p.sortDir
      ) + paginationHTML(p);
    } catch (e) { node.innerHTML = emptyState(e.message); }
    renderIcons();
  ...
  async function openProductDetail(product) {
    ...
    const config = await getMenuConfig('products');
    const visible = (config?.fields || []).filter(f => f.status === 'ACTIVE' && f.visibleDetail);
    const fieldByCode = { code: fields[0], name: fields[1], specification: fields[5], productModel: fields[6], customerPartNumber: fields[7], minPackagingQty: fields[8], drawingNumber: fields[9], brand: fields[10], color: fields[11], productGroup: fields[12], productOldId: fields[13], productHierarchy: fields[14], divisionCode: fields[15], manufacturerNumber: fields[16], manufacturerPartNumber: fields[17], materialRevisionLevel: fields[18], serialNumberProfile: fields[19], grossWeight: fields[20], netWeight: fields[21], countryOfOrigin: fields[22], batchManaged: fields[23], traceable: fields[24], markedForDeletion: fields[25] };
    const detailFields = visible.length ? visible.map(f => [localizedMeta(f), fieldByCode[f.fieldCode]?.[1] ?? readPath(value, f.fieldPath)]) : fields;
    openDrawer(`${t('products')} · ${value.code}`, t('productSubtitle'), `<div class="drawer-body">${detailGrid(detailFields)}</div>`);
  }

  // Batch Detail
  function openBatchDetail(value) {
    const fields = [
      [t('batchNo'), value.batchNo], [t('product'), `${value.productCode || ''} ${value.productName || ''}`],
      [t('plant'), value.plant], [t('batchStatus'), statusPill(value.batchStatus)],
      [t('quantity'), `${value.quantity || '—'} ${value.unit || ''}`],
      [t('manufactureDate'), formatDate(value.manufactureDate)], [t('expirationDate'), formatDate(value.expirationDate)],
      [t('shelfLifeExpirationDate'), formatDate(value.shelfLifeExpirationDate)],
      [t('supplierBatch'), value.supplierBatch], [t('inspectionLot'), value.inspectionLot],
      [t('lastSync'), formatDate(value.sapLastSyncAt)]
    ];
    openDrawer(`${t('batches')} · ${value.batchNo}`, t('batchSubtitle'), `<div class="drawer-body">${detailGrid(fields)}</div>`);
  }

  // Work Order Detail - tabbed view: Summary | Components | Operations
  async function openOrderDetail(order) {
    ...
    const compTable = components.length ? `<div class="table-wrap">${dataTable([t('sequence'), t('material'), t('requiredQuantity'), t('withdrawn'), t('available'), t('unit'), t('reservation'), t('requirementDate'), t('goodsMovement')], components.map(v => `<tr><td>${escVal(v.sequenceNo)}</td><td><span class="cell-title">${escVal(v.productCode)}</span><span class="cell-sub">${escVal(v.productName || v.itemDescription)}</span></td><td>${escVal(v.requiredQuantity)}</td><td>${escVal(v.withdrawnQuantity)}</td><td>${escVal(v.availableQuantity)}</td><td>${escVal(v.unit)}</td><td>${escVal(v.reservationNo)} / ${escVal(v.reservationItem)}</td><td>${escVal(v.requirementDate)}</td><td>${escVal(v.goodsMovementType)}</td></tr>`))}</div>` : `<div class="empty">${t('noData')}</div>`;
    const opTable = operations.length ? `<div class="table-wrap">${dataTable([t('sequence'), t('operation'), t('workCenter'), t('plant'), t('plannedYield'), t('confirmedYield'), t('status')], operations.map(v => `<tr><td>${escVal(v.sequenceNo)}</td><td><span class="cell-title">${escVal(v.operationCode)}</span><span class="cell-sub">${escVal(v.operationNameZh || v.operationNameEn || v.operationNameAr)}</span></td><td>${escVal(v.workCenterCode)}</td><td>${escVal(v.plant)}</td><td>${escVal(v.plannedYieldQuantity || v.plannedQuantity)}</td><td>${escVal(v.confirmedYieldQuantity || v.completedQuantity)}</td><td>${statusPill(v.status)}</td></tr>`))}</div>` : `<div class="empty">${t('noData')}</div>`;
    openDrawer(`${t('workOrders')} · ${order.orderNo}`, t('orderSubtitle'),
      `<div class="drawer-body">
        <div class="tabs" id="order-detail-tabs">
          <button class="tab active" data-tab="summary">${t('overview')} (${summary.length})</button>
          <button class="tab" data-tab="components">${t('components')} (${components.length})</button>
          <button class="tab" data-tab="operations">${t('operations')} (${operations.length})</button>
        </div>
        <div class="tab-content" id="tab-summary">${detailGrid(summary)}</div>
        <div class="tab-content hidden" id="tab-components">${compTable}</div>
        <div class="tab-content hidden" id="tab-operations">${opTable}</div>
      </div>`);
    // Tab switching
    $$('#order-detail-tabs .tab').forEach(tab => {
      tab.addEventListener('click', () => {
        $$('#order-detail-tabs .tab').forEach(t => t.classList.remove('active'));
  ...
  async function openMenuConfig(menu) {
  ...
  if (state.token) {
    api('/auth/me').then(payload => {
      state.user = payload.data;
      showWorkspace(payload.data);
      loadNavigation().finally(() => renderView('overview'));
    }).catch(() => logout(false));
  }
})();
